...
export async function POST(request: NextRequest) {
  try {
    // Import config and utilities at runtime, not at module level
    const { config } = await import('@/lib/config')
    const { createSession, checkBruteForce, recordFailedAttempt, clearFailedAttempts } = await import('@/lib/auth')
